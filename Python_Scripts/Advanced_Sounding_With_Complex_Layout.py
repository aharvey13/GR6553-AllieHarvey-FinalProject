...
###########################################
#To pull the data, I combined codes from the advanced sounding plot with complex layout and the simple skew-t sounding from Metpy
#Pulling data came from the simple sounding since it was easier to pull data using the datetime and siphon features 
#Datatime involves including the year, month, day, and zulu time of launch; station is the three letter identifier
col_names = ['pressure', 'height', 'temperature', 'dewpoint', 'direction', 'speed']
dt = datetime(2020, 10, 12, 12)
station = 'GRB'
# Read remote sounding data based on time (dt) and station
df = WyomingUpperAir.request_data(dt, station)

#At this point, I let the Metpy example take over - shoutout to Metpy

# Drop any rows with all NaN values for T, Td, winds
df = df.dropna(subset=('temperature', 'dewpoint', 'direction', 'speed'),
               how='all').reset_index(drop=True)
###########################################
# We will pull the data into individual variables and assign units.
...
# add the Metpy logo - I took it out of my presentation to make the skew-t easier to read
...
plt.figtext(0.45, 0.97, 'GRB | OCT 12TH 2020- 12Z VERTICAL PROFILE',
            weight='bold', fontsize=20, ha='center')

# Show the plot
#plt.show()
plt.savefig('GRB_Sounding_10.12_12Z.png')
